...
const institutionMembershipSchema = new mongoose.Schema(
  {
    ...
    studentDetails: {
      type: new mongoose.Schema(
        {
          rollNumber: { type: String, trim: true, maxlength: 100 },
          ...
          graduationYear: {
            ...
            validate: {
              ...
              message: "graduationYear must be an integer",
            },
          },
        },
...
institutionMembershipSchema.index({ applicationId: 1 }, { unique: true });
institutionMembershipSchema.index({ userId: 1, institutionId: 1 }, { unique: true });
institutionMembershipSchema.index({ institutionId: 1, status: 1 });

institutionMembershipSchema.pre("validate", function (next) {
...
institutionMembershipSchema.statics.allowedTransitions = { active: ["inactive"], inactive: ["active"] };
institutionMembershipSchema.statics.canTransition = function (from, to) {
  return this.allowedTransitions[from]?.includes(to) || false;
};
institutionMembershipSchema.pre(["findOneAndUpdate", "updateOne"], async function (next) {
